...
pub fn add_glyph_to_atlas(
        ...
        .any(|atlas| add_char_to_font_atlas(atlas).is_ok())
    {
        ...
        // STELLWERK PATCH (vendored, upstream value: 512): texts whose glyphs
        // span MULTIPLE atlas pages render corrupted in bevy 0.18 (glyph
        // quads sample the wrong page's texture — wrong-size letters). A
        // German UI with umlauts exceeds a 512² page per font size once the
        // 4 subpixel-bin variants per glyph pile up; 2048² gives every size
        // ~16× the capacity, so a second page never appears in practice.
        // Pick the higher of 2048 or the smallest power of 2 greater than glyph_max_size
        let containing = (1u32 << (32 - glyph_max_size.leading_zeros())).max(2048);
