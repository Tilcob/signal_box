...
| 2026-06-15 | M2-Restfeature 04 (plans/M2/restfeatures/04), TEILWEISE: Die **Schriftprüfung** aus §10/§3-DoD existiert jetzt als `font::tests::shipped_font_covers_all_ui_glyphs` (Glyph-Abdeckung über beide i18n-Tabellen inkl. Umlaute/ß + hartkodierte UI-Symbole; `ttf-parser` dev-only). Bewacht jeden Font-Tausch. Der DIN-artige Font selbst bleibt offen (OFL-Binärdatei = Asset-Drop), bis dahin `DejaVuSansMono` mit voller Abdeckung. |
