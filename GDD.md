...
| 2026-06-15 | M2-Restfeature 04 (plans/M2/restfeatures/04): Schriftprüfung (`font::tests::shipped_font_covers_all_ui_glyphs`, `ttf-parser` dev-only) PLUS DIN-artige Schrift **Saira Semi Condensed** (OFL) eingebunden, DejaVu entfernt. Saira ist eine reine Textschrift ohne Symbol-Glyphen → Status-Icons (Medaillen, „gelöst") als gezeichnete `bevy_ui`-Shapes (`widgets::dot`), ▶◀✗ ersetzt durch »«× (Weg A). Echte PNG-Icons + Atlas-Sichtcheck vor Release. Schließt M2-§8-Abweichung 4 (Schrift). |
| 2026-06-22 | M3 re-scoped: Eine **Itch.io Early Demo** (Feedback-Build, ~20 Level, Polish-Pass) wird dem vollen Steam-M3 vorgeschaltet. Steam-Page/SteamPipe/Next-Fest/Capsule/Trailer, das `demo`-Cargo-Feature und das volle §9-Form-/Muster-Barrierefreiheitssystem wandern in den späteren Steam-Push (M3-spät/M4). §13 inhaltlich unberührt — Steam-M3 bleibt das Ziel, die Itch-Demo ist ihm nur vorgeschaltet. Polish-Entscheidungen (Anrede „Du", Farbenblind-Pass minimal) und Details: plans/M3/M3-demo.md. |
